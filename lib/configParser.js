...
exports.parse = function() {
	...
	if (Object.prototype.toString.call(config.apiId) === '[object Array]') {
		...
		if (config.apiId.length !== config.apiKey.length) {
		    if (config.apiId.length === 0 || config.apiKey.length === 0) {
		        logger.error('Configuration missing: have you edited the defaults in "lib/config.js"?');
		    } else {
    ...
    if (exports.maxFileExpirationEnabled === undefined) { exports.maxFileExpirationEnabled = true; }
	exports.debug = config.debug;
    if (exports.debug === undefined) { exports.debug = false; }
